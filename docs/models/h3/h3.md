...
denoises a packed text + video + audio sequence and emits video with native stereo audio. miles-diffusion currently
trains the **t2va** (text-to-audio-video) workflow, video branch only — the audio stream is rolled out but not trained.

The shipped recipe is a 2-GPU verification path (LoRA + LoRA-IPC, PickScore). A large-scale recipe is coming soon.
...
Canonical recipe: `scripts/run_diffusion_grpo_h3_t2va_2gpu.py` — 2 colocate GPUs, reward colocated, 16:9 / 4 s
...
**Large-scale recipe coming soon.** The commands above are the 2-GPU verification path. A multi-GPU recipe
...
This is the batch-4 curve, not the forthcoming large-scale recipe.
