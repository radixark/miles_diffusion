...
def test_mean_weights_items_not_calls():
    ...
    metrics.emit_mean("loss", torch.tensor(6.0), count=3)
    metrics.emit_mean("loss", torch.tensor(1.0), count=1)
...
def test_declared_but_never_recorded_metrics_are_dropped():
    ...
    metrics.emit_mean("loss", torch.tensor(4.0), count=2)
    assert metrics.reduce() == {"loss": 2.0}


def test_zero_count_contribution_does_not_create_a_datapoint():
    metrics = _buffer(loss=MetricReduce.MEAN)
    metrics.emit_mean("loss", torch.tensor(0.0), count=0)
...
def test_rejects_undeclared_metric():
    ...
    with pytest.raises(KeyError):
        metrics.emit_mean("typo", torch.tensor(1.0), count=1)


def test_mean_count_is_keyword_only():
    metrics = _buffer(loss=MetricReduce.MEAN)
    with pytest.raises(TypeError):
        metrics.emit_mean("loss", torch.tensor(1.0), 1)
...
def test_rejects_non_scalar_values():
    ...
    with pytest.raises(RuntimeError):
        metrics.emit_mean("loss", torch.zeros(4), count=4)
...
def test_accumulates_in_float64_so_long_runs_do_not_drift():
    ...
    metrics = _buffer(loss=MetricReduce.MEAN)
    for _ in range(4096):
        metrics.emit_mean("loss", value, count=1)
