...
def test_mean_weights_items_not_calls():
    ...
    metrics.emit_mean(key="loss", total=torch.tensor(6.0), count=3)
    metrics.emit_mean(key="loss", total=torch.tensor(1.0), count=1)
    assert metrics.reduce() == {"loss": 7.0 / 4.0}


def test_max_keeps_the_largest():
    metrics = _buffer(abs_diff=MetricReduce.MAX)
    metrics.emit_max(key="abs_diff", value=torch.tensor(0.25))
    metrics.emit_max(key="abs_diff", value=torch.tensor(2.5))
    metrics.emit_max(key="abs_diff", value=torch.tensor(1.0))
...
def test_replicated_passes_the_value_through():
    ...
    metrics.emit_replicated(key="grad_norm", value=torch.tensor(0.125))
...
def test_declared_but_never_recorded_metrics_are_dropped():
    ...
    metrics.emit_mean(key="loss", total=torch.tensor(4.0), count=2)
    assert metrics.reduce() == {"loss": 2.0}


def test_zero_count_contribution_does_not_create_a_datapoint():
    metrics = _buffer(loss=MetricReduce.MEAN)
    metrics.emit_mean(key="loss", total=torch.tensor(0.0), count=0)
...
def test_rejects_undeclared_metric():
    ...
    with pytest.raises(KeyError):
        metrics.emit_mean(key="typo", total=torch.tensor(1.0), count=1)


def test_emit_op_must_match_schema():
    metrics = _buffer(loss=MetricReduce.MEAN)
    with pytest.raises(ValueError, match="reduces as MEAN, not MAX"):
        metrics.emit_max(key="loss", value=torch.tensor(1.0))
...
def test_rejects_non_scalar_values():
    ...
    with pytest.raises(RuntimeError):
        metrics.emit_mean(key="loss", total=torch.zeros(4), count=4)
...
def test_accumulates_in_float64_so_long_runs_do_not_drift():
    ...
    metrics = _buffer(loss=MetricReduce.MEAN)
    for _ in range(4096):
        metrics.emit_mean(key="loss", total=value, count=1)
