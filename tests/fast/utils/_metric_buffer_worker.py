...
def case_mean_weights_items_not_ranks() -> None:
    ...
    for _ in range(4 if rank == 0 else 1):
        metrics.emit_mean(key="loss", total=torch.tensor(2.0 if rank == 0 else 0.0), count=1)
...
def case_max_is_a_real_max() -> None:
    ...
    metrics.emit_max(key="abs_diff", value=torch.tensor(100.0 if rank == dist.get_world_size() - 1 else 0.5))
...
def case_metric_only_one_rank_records() -> None:
    ...
    if dist.get_rank() == 0:
        metrics.emit_mean(key="phase_high_noise", total=torch.tensor(6.0), count=3)
        metrics.emit_max(key="worst", value=torch.tensor(7.0))
...
def case_reduces_on_the_given_group_only() -> None:
    ...
    metrics.emit_mean(key="loss", total=torch.tensor(1.0 if rank < 2 else 100.0), count=1)
