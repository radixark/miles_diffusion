...
the runner's GPU set (the 5gpu runner exposes five). --extra-args
--deterministic-mode is what makes the strict comparison viable: the recipe
ships --diffusion-debug-mode but, unlike the CI-only recipes, does not pin
determinism itself, and cosmos3 leaves --fsdp-attention-backend unset, so SDPA
is covered by torch's global flag.
