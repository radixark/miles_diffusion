...
--cuda-visible-devices "" restores the framework default: this recipe is the
one that hardcodes 0,1,2,3, which would pin the raylet to a fixed four cards
instead of the set the runner handed out. --extra-args --deterministic-mode is
what makes the strict comparison viable: the recipe ships --diffusion-debug-mode
but, unlike the CI-only recipes, does not pin determinism itself, and cosmos3
leaves --fsdp-attention-backend unset, so SDPA is covered by torch's global flag.
