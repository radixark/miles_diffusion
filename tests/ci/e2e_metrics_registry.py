...
A test is one register_e2e_ci(...) call (suite placement parsed via AST by
ci_register.py; executing the file runs `script` and checks `metrics`).
Standards live at tests/ci/fixtures/e2e_standards/<test_stem>.json and are
updated by the PR author, never by CI:
...
Each test maps to exactly one metrics recording (logs/e2e_metrics/<test_stem>.jsonl)
and one standard; --metrics only overrides the source for registering an
exploratory run (example scripts record to logs/<RUN_NAME>/metrics.jsonl).
"""
...
def register_e2e_ci(
    ...
    *,
    ...
    """Run `script` with metric recording and check `metrics` against the
    standard; suite args are consumed by the AST parser and unused here."""
...
def check_or_update(
    ...
    for m in metrics:
        ...
        for i, ((gs, gv), (ws, wv)) in enumerate(zip(got_series, want_series)):
            if gs != ws:
