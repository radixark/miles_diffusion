...
async def generate_microgroup(
    ...
    """Run one ``POST /rollout/generate`` for a microgroup sharing a prompt and seed base."""
...
async def generate_rollout_async(
    ...
    """Generate ``rollout_batch_size`` groups, dropping the ones the dynamic filter rejects."""
...
async def eval_rollout_single_dataset(
    ...
    """Generate ``n_samples_per_eval_prompt`` samples for every prompt of one eval dataset."""
    assert not args.group_rm, "Group RM is not supported for eval rollout"
...
def generate_rollout(
    ...
    """Entry point named by ``--rollout-function-path``: run one train or eval rollout."""
    assert args.rollout_global_dataset
    if evaluation:
